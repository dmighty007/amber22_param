...
int main() {
    ...
    printf("Testing proper treatment of a bad prmtop; an error message is expected:\n");
...
*/
    ...
    get_box(&box[0], &box[1], &box[2], &box[3], &box[4], &box[5]);

    failed += compare(box[0], 51.0, "a-vector length");
    failed += compare(box[1], 51.0, "a-vector length");
    failed += compare(box[2], 51.0, "b-vector length");
    failed += compare(box[3], 90.0, "alpha angle");
    failed += compare(box[4], 90.0, "beta angle");
    failed += compare(box[5], 90.0, "gamma angle");
...
int slko_files_exist(void) {
    ...
    test = fopen("../../dat/slko/mio-1-1/C-C.skf", "r");
    if (test == NULL)
        return 0;
    else
        fclose(test);
