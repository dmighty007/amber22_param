...
set_SANDER_CMD()
{
	...
	if [ `echo $((${NUMPROCS} % $1))` -eq 0 ]; then
            NUMPROCS_is_okay="yes it is"
        fi

        if [[ $SANDER == *"sander"* ]]; then
            MX=$1
        elif [[ $SANDER == *"pmemd"* ]]; then
		MX=`echo $(($1 + $1))`
        fi

        if [ ${NUMPROCS} -lt $MX ]; then
            NUMPROCS_is_okay="not enough"
        fi

        if [ "${NUMPROCS_is_okay}" != "yes it is" ] ; then
            echo " This test case (`pwd`) requires a least $MX mpi threads."
            echo " The number of mpi threads must also be a multiple of $1"
            echo " and not more than `expr $1 \* 64`. Not running test, exiting ....."
            exit 1
        fi
    fi
