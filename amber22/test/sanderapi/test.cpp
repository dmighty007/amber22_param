...
*/
    ...
    failed = compare(box[0], 51.0, "a-vector length") || failed;
    failed = compare(box[1], 51.0, "a-vector length") || failed;
    failed = compare(box[2], 51.0, "b-vector length") || failed;
    failed = compare(box[3], 90.0, "alpha angle") || failed;
    failed = compare(box[4], 90.0, "beta angle") || failed;
    failed = compare(box[5], 90.0, "gamma angle") || failed;
...
bool slko_files_exist(void) {
    ...
    if (ifstream("../../dat/slko/mio-1-1/C-C.skf", ifstream::in))
        return true;
    return false;
}
