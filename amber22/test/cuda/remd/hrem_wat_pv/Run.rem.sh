...
#TEST-DESCRIP Test Hamiltonian replica exchange calc, NPT, should match CPU closely. 
...
Title "Two replica PME NPT HREMD test."
...
Title Line                
 ...
 &end
EOF
  echo "-O -rem 3 -remlog $log -i rem.in.$REP -p $TOP -c $CRD -o ./$output.$REP -inf reminfo.$REP -r ./rem.r.$REP" >> groupfile
done

$DO_PARALLEL $TESTsander -O -ng 2 -groupfile groupfile < /dev/null || error

../../../dacdif $log.save $log
../../../dacdif -a 0.00011 rem.out.000.save rem.out.000
../../../dacdif -a 0.00011 rem.out.001.save rem.out.001
