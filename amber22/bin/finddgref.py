...
group.add_argument('-mdexec', dest='mdexec', metavar='FILE',
                   help='Path to the AMBER executable file. Example: $AMBERHOME/bin/pmemd',
...
group.add_argument('-bin-path', dest='binpath', metavar='FILE', required=False,
                   ...
                   type=str, default='')
group = parser.add_argument_group('AMBER Arguments - Global', 'These arguments apply to both simulations with or without Replica Exchange.')
group.add_argument('-AllowSmallBox', dest='AllowSmallBox', action='store_const',
                   help='''If stated, sets the flag AllowSmallBox in the simulation command (may be needed by pmemd.cuda)''',
                   const=True, default=False)
...
def RunAMBER(opt,log,data,equi):
    ...
    asb=""
    if (opt.AllowSmallBox):
        asb="-AllowSmallBox"
    if (data.remd):
        cmd = "%s %s %s -ng %d -groupfile %s.temporary" % (opt.do_parallel,opt.mdexec,asb,opt.ng,opt.groupfile)
    else:
        if (equi == 0):
            crd_md = opt.inpcrd
            if (data.modeopt == 1):
            ...
            cprst_md = opt.cprestrt
            cerst_md = opt.cerestrt
        cmd = "%s %s -O -i %s.temporary -p %s -c %s -x %s -inf %s -o %s -r %s" % (opt.mdexec,asb,opt.mdin,opt.prmtop,crd_md,opt.mdcrd,opt.mdinfo,opt.mdout,opt.restrt)
        if (data.modeopt == 1):
...
def EstimateDG(opt,log,data):
    ...
    while (data.step <= opt.maxsteps):
        ...
        if (data.frac>0.99 or data.frac<0.01):
            if (data.modeopt == 1):
                txt1 = "protonated"
            else:
                txt1 = "reduced"
            ErrorExit(opt,log,'The fraction of %s species is %.2f%%. This value should not have been obtained during this stage of the executation. Check your input files and input parameters.' % (txt1,data.frac))
...
def main(opt):
    ...
    # Else, we try to find one value of DELTAGREF where 0.2 > data.frac > 0.8
        ...
        if (data.remd):
            data.num = int(round(float(data.numexchg)/10.0))
        else:
            data.num = int(round(float(data.nstlim)/10.0))
        while (data.step <= opt.maxsteps):
