...
#TEST-DESCRIP Four replica GB Boltzmann Reservoir REMD.
...
. ../CudaRemdCommon.sh
Title "Four replica GB Boltzmann Reservoir REMD test."
...
CRD=tc5b.crd
i=0
for TEMP0 in "281.4" "300.0" "319.8" "340.9" ; do
  EXT=`printf "%03i" $i`
  cat > rem.in.$EXT <<EOF
Trp-cage GB REMD
&cntrl
...
$DO_PARALLEL $TESTsander -O -ng 4 -groupfile groupfile -rem 1 -remlog rem.log -rremd 1 -reservoir reservoir.nc
CheckError $? "${0}"
